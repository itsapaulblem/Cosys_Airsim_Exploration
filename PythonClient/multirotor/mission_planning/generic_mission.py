...
class GenericMission(ABC):
    ...
    def __init__(self, altitude=20, speed=5, disable_collision_detection=False):
        ...
        self.collision_detection_enabled = not disable_collision_detection  # Can be temporarily disabled
        self.collision_detection_disabled_globally = disable_collision_detection  # Global disable flag
        ...
        # Photo organization
        self.mission_instance_folder = None  # Will be set during initialization
        self.mission_start_datetime = None  # Will be set during initialization
    ...
    def _setup_photo_directories(self):
        """Setup photo directory structure for this mission instance."""
        try:
            # Get mission type name (class name without 'Mission' suffix)
            mission_type = self.__class__.__name__.replace('Mission', '').lower()
            
            # Create base photos directory in mission_planning folder
            base_photos_dir = os.path.join(os.path.dirname(__file__), "photos")
            mission_type_dir = os.path.join(base_photos_dir, mission_type + "_mission")
            
            # Create mission type directory if it doesn't exist
            os.makedirs(mission_type_dir, exist_ok=True)
            
            # Find next run number
            existing_runs = [d for d in os.listdir(mission_type_dir) 
                           if d.startswith("Run_") and os.path.isdir(os.path.join(mission_type_dir, d))]
            ...
            # Create timestamped instance folder
            timestamp = self.mission_start_datetime.strftime("%Y-%m-%d_%H-%M-%S")
            instance_folder_name = f"Run_{next_run_num}_{timestamp}"
            self.mission_instance_folder = os.path.join(mission_type_dir, instance_folder_name)
            
            # Create the instance folder
            os.makedirs(self.mission_instance_folder, exist_ok=True)
            
            print(f"Photos will be saved to: {self.mission_instance_folder}")
            
        except Exception as e:
            print(f"Warning: Could not setup photo directories: {str(e)}")
            # Fallback to simple photos directory
            self.mission_instance_folder = os.path.join(os.path.dirname(__file__), "photos")
            os.makedirs(self.mission_instance_folder, exist_ok=True)
    ...
    def _initialize_mission(self):
        ...
        self.mission_start_datetime = datetime.now()
        self.phase_times = {
        ...
        self._setup_photo_directories()
        
        print("Mission initialized")
        if self.collision_detection_disabled_globally:
            print("⚠️  COLLISION DETECTION GLOBALLY DISABLED")
    ...
    def _check_collision(self):
        ...
        # Skip collision detection if globally disabled
        if self.collision_detection_disabled_globally:
            return
    ...
    def disable_collision_detection(self):
        ...
        if not self.collision_detection_disabled_globally:
            self.collision_detection_enabled = False
            print(f"Collision detection temporarily disabled")
        else:
            print(f"Collision detection already globally disabled")
    
    def enable_collision_detection(self):
        """Re-enable collision detection."""
        if not self.collision_detection_disabled_globally:
            self.collision_detection_enabled = True
            print(f"Collision detection re-enabled")
        else:
            print(f"Collision detection globally disabled - cannot re-enable temporarily")
    ...
    def take_photo(self, filename_prefix="photo", camera_name="0"):
        """Take a photo and save it to the mission instance directory."""
        # Don't take photos if emergency landing is triggered
        if self.emergency_landing_triggered:
            return None
            
        try:
            # Import numpy and cv2 for uncompressed image handling
            import numpy as np
            import cv2
            
            # Use mission instance folder if available, otherwise fallback
            photo_dir = self.mission_instance_folder if self.mission_instance_folder else "photos"
            ...
            # Take photo with proper format handling - using the working camera configuration
            ...
            if responses and len(responses) > 0:
                ...
                print(f"  Image response - Type: {response.image_type}, Width: {response.width}, Height: {response.height}")
                print(f"  Pixels as float: {response.pixels_as_float}, Compress: {response.compress}")
                
                # Handle different image formats properly - following hello_drone.py pattern
                if response.pixels_as_float:
                    # For float images (depth, etc.), save as PFM
                    pfm_filepath = filepath.replace('.png', '.pfm')
                    print(f"  Float image - size: {len(response.image_data_float)}")
                    airsim.write_pfm(pfm_filepath, airsim.get_pfm_array(response))
                    print(f"Photo saved (PFM): {os.path.basename(pfm_filepath)}")
                    return pfm_filepath
                    
                elif response.compress:  # PNG format - compressed
                    # For compressed PNG images
                    print(f"  Compressed PNG - size: {len(response.image_data_uint8)}")
                    if len(response.image_data_uint8) > 0:
                        ...
                        if os.path.exists(filepath) and os.path.getsize(filepath) > 0:
                            print(f"Photo saved (compressed): {os.path.basename(filename)} ({os.path.getsize(filepath)} bytes)")
                            return filepath
                        else:
                            error_msg = f"Compressed photo file was not created properly: {filepath}"
                            self.errors.append(error_msg)
                            print(f"  {error_msg}")
                            return None
                    else:
                        error_msg = "Empty compressed image data received"
                        self.errors.append(error_msg)
                        print(f"  {error_msg}")
                        return None
                        
                else:  # Uncompressed array
                    # For uncompressed RGB arrays
                    print(f"  Uncompressed array - size: {len(response.image_data_uint8)}")
                    if len(response.image_data_uint8) > 0:
                        ...
                        if success and os.path.exists(filepath) and os.path.getsize(filepath) > 0:
                            print(f"Photo saved (uncompressed): {os.path.basename(filename)} ({os.path.getsize(filepath)} bytes)")
                            return filepath
                        else:
                            error_msg = f"Uncompressed photo save failed: {filepath}"
                            self.errors.append(error_msg)
                            print(f"  {error_msg}")
                            return None
                    else:
                        error_msg = "Empty uncompressed image data received"
                        self.errors.append(error_msg)
                        print(f"  {error_msg}")
                        return None
            else:
                error_msg = "No image response received"
                self.errors.append(error_msg)
                print(f"  {error_msg}")
                return None
                
        except ImportError as e:
            error_msg = f"Missing required libraries (numpy/cv2): {str(e)}"
            self.errors.append(error_msg)
            print(f"  {error_msg}")
            return None
        except Exception as e:
...
def create_standard_argument_parser():
    ...
    parser.add_argument('--disable_collision_detection', action='store_true',
                       help='Completely disable collision detection for the entire mission')
    return parser
