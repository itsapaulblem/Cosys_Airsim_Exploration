...
class BoxMission(GenericMission):
    ...
    def __init__(self, box_size=400, altitude=20, speed=5, orbit_radius=50, 
                 ...
                 enable_orbits=True, disable_collision_detection=False, camera_name="0"):
        """
        ...
        Args:
            ...
            disable_collision_detection: Whether to disable collision detection for the entire mission
            camera_name: Camera name/ID to use for photos (default: "0")
        """
    ...
    def print_mission_parameters(self):
        ...
        if self.enable_orbits:
            ...
            print(f"Collision Detection: {'GLOBALLY DISABLED' if self.collision_detection_disabled_globally else 'Enabled for transit, disabled during orbital photography'}")
        else:
            print(f"Mission Type: Simple box pattern flight (no orbital photography)")
            print(f"Collision Detection: {'GLOBALLY DISABLED' if self.collision_detection_disabled_globally else 'Enabled throughout mission'}")
    ...
    def _perform_orbital_photography(self, center_x, center_y, center_z, vertex_num):
        """Perform orbital photography around a vertex using continuous velocity control."""
        print(f"  Starting orbital photography at vertex {vertex_num}")
        ...
        try:
            ...
            print(f"  Starting smooth orbital motion")
            ...
            # Continuous velocity-based orbital motion
            while True:
                ...
                # Calculate target position for smooth orbital motion
    ...
    def _point_towards_center_smooth(self, current_x, current_y, center_x, center_y):
        """Point the drone towards the center with ultra-smooth yaw control."""
        try:
            # Calculate angle to center
            dx = center_x - current_x
            dy = center_y - current_y
...
def main():
    ...
    parser.add_argument('--camera_name', type=str, default='0',
                       help='Camera name/ID to use for photos (default: "0")')
