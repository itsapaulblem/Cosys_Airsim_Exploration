...
def setup_thermal_targets(client):
    """Set up thermal targets for detection"""
    try:
        # Try different methods to set object temperature
        target_objects = ["SK_SciFiSoldier02", "SK_SciFiSoldier01", "SK_SciFiSoldier03", "Target", "Human"]
        
        for obj in target_objects:
            try:
                # Method 1: Try simSetObjectMaterial
                if hasattr(client, 'simSetObjectMaterial'):
                    client.simSetObjectMaterial(obj, "Temperature", 10.0)
                    print(f"Set {obj} temperature to 10°C using simSetObjectMaterial")
                # Method 2: Try the original method with try/catch
                elif hasattr(client, 'simSetObjectMaterialFromName'):
                    client.simSetObjectMaterialFromName(obj, "Temperature", 10.0)
                    print(f"Set {obj} temperature to 10°C using simSetObjectMaterialFromName")
                else:
                    print(f"No material setting method available for {obj}")
            except Exception as obj_error:
                print(f"Could not set temperature for {obj}: {obj_error}")
        
        # Alternative: Try to set via environment settings
        try:
            # Set a high ambient temperature to make objects warmer
            if hasattr(client, 'simSetWeatherParameter'):
                client.simSetWeatherParameter(airsim.WeatherParameter.Fog, 0.0)
                print("Set weather parameters")
        except Exception as weather_error:
            print(f"Could not set weather: {weather_error}")
            
    except Exception as e:
        print(f"Could not set object temperatures: {e}")

def detect_hot_target(client, camera_names, threshold=5):
    for cam in camera_names:
        ...
        print(f"Thermal image stats for {cam}: min={gray.min()}, max={gray.max()}, mean={gray.mean():.1f}")
        
        # Look for pixels that are brighter than the background
        mean_val = gray.mean()
        std_val = gray.std()
        bright_pixels = np.sum(gray > mean_val + std_val)
        
        if bright_pixels > 100:  # If more than 100 pixels are significantly brighter than average
            print(f"Found {bright_pixels} bright pixels in {cam}")
            return True, cam
...
def move_and_check(client, x, y, z, speed, camera_names, home_pos, home_lat, home_lon):
    ...
    time.sleep(0.5)
    found, cam = detect_hot_target(client, camera_names)
    if found:
        ...
        # Take thermal camera image 
        thermal_response = client.simGetImages([
            airsim.ImageRequest(cam, airsim.ImageType.Infrared, False, False)
        ])[0]
        if thermal_response.width > 0:
            thermal_img_bytes = thermal_response.image_data_uint8
            thermal_img1d = np.frombuffer(thermal_img_bytes, dtype=np.uint8)
            thermal_img = thermal_img1d.reshape(thermal_response.height, thermal_response.width, 3)
            thermal_filename = f"target_thermal_{cam}_{int(time.time())}.png"
            cv2.imwrite(thermal_filename, thermal_img)
            print(f"Saved thermal image: {thermal_filename}")
        
        # Take normal camera image from corresponding normal camera
        normal_cam = cam.replace("thermal_", "")  # Remove "thermal_" prefix to get normal camera name
        normal_response = client.simGetImages([
            airsim.ImageRequest(normal_cam, airsim.ImageType.Scene, False, False)
        ])[0]
        if normal_response.width > 0:
            normal_img_bytes = normal_response.image_data_uint8
            normal_img1d = np.frombuffer(normal_img_bytes, dtype=np.uint8)
            normal_img = normal_img1d.reshape(normal_response.height, normal_response.width, 3)
            normal_filename = f"target_normal_{normal_cam}_{int(time.time())}.png"
            cv2.imwrite(normal_filename, cv2.cvtColor(normal_img, cv2.COLOR_RGB2BGR))
            print(f"Saved normal camera image: {normal_filename}")
...
def search_and_land_spiral(client, ned_bounds, speed, altitude, step, max_attempts=50, x_resolution=10.0):
    ...
    home_lon = gps_data.gnss.geo_point.longitude
    home_pos = client.getMultirotorState().kinematics_estimated.position
    camera_names = ["thermal_front_center", "thermal_back_center", "thermal_front_left", "thermal_front_right", "thermal_bottom_center"]
    attempt_count = 0
    left = min_x
    right = max_x
    bottom = min_y
...
def main():
    ...
    try:
        ...
        # Set up thermal targets after connection
        setup_thermal_targets(client)
