...
set "DOCKERFILE_PATH=VNC\Dockerfile.ros2_vnc"
set "BUILD_CONTEXT=..\..\."
set "VNC_PORT=5901"
set "ROS_DOMAIN_ID=0"
...
echo     %~nx0 run --launch-rviz --vnc-port 5902
...
docker ps -a --format "{{.Names}}" | findstr /r /c:"^%CONTAINER_NAME%$" >nul 2>&1
if not errorlevel 1 (
...
call :print_status "VNC will be available at: localhost:%VNC_PORT% (use VNC client)"
...
call :print_status "Volume Mount ROS2: %VOLUME_MOUNT_ROS2%"
...
if "%DETACHED%"=="" (
    ...
    call :print_status "Access VNC at: localhost:%VNC_PORT% (use VNC client)"
    call :print_status "Default VNC password: ubuntu"
    call :print_status "Default directory: /airsim_ros2_ws"
    if /i "%VOLUME_MOUNT_ROS2%"=="true" (
        call :print_status "ROS2 source mounted - changes will persist!"
        call :print_status "Use 'build_interfaces' or 'build_workspace' commands"
    )
)
exit /b 0
...
REM Check if container is running using docker inspect
docker inspect -f "{{.State.Running}}" "%CONTAINER_NAME%" 2>nul | findstr "true" >nul 2>&1
if not errorlevel 1 (
    call :print_status "Opening bash shell in container: %CONTAINER_NAME%"
...
REM Remove image
docker images --format "{{.Repository}}:{{.Tag}}" | findstr /r /c:"^%IMAGE_NAME%:%IMAGE_TAG%$" >nul 2>&1
if not errorlevel 1 (
...
REM Check if container exists using docker inspect (more reliable)
docker inspect "%CONTAINER_NAME%" >nul 2>&1
if not errorlevel 1 (
    call :print_status "Container '%CONTAINER_NAME%' exists"
    
    REM Check if running using docker inspect
    docker inspect -f "{{.State.Running}}" "%CONTAINER_NAME%" 2>nul | findstr "true" >nul 2>&1
    if not errorlevel 1 (
        call :print_status "Status: RUNNING"
        call :print_status "VNC Access: localhost:%VNC_PORT% (use VNC client)"
...
REM Check if image exists using docker inspect (more reliable)
docker inspect "%IMAGE_NAME%:%IMAGE_TAG%" >nul 2>&1
if not errorlevel 1 (
