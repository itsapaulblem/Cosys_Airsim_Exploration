...
class BoxMission(GenericMission):
    ...
    def __init__(self, box_size=400, altitude=20, speed=5, orbit_radius=50, 
                 orbit_speed=3, photos_per_orbit=8, orbit_mode="velocity", max_collisions=5, enable_orbits=True):
        """
        ...
        Args:
            ...
            enable_orbits: Whether to perform orbital photography at vertices
        """
    ...
    def print_mission_parameters(self):
        ...
        if self.enable_orbits:
            ...
            print(f" Orbital Photography: Each vertex will be the START (0°) of the orbit circle")
            print(f"   • Drone flies to vertex and begins circular orbit from that point")
            print(f"   • Circle center is offset by {self.orbit_radius}m west of each vertex")
        else:
            print(f" Orbital Photography: DISABLED - Simple box pattern only")
        ...
        for i, (x, y) in enumerate(self.vertices):
            orbit_info = f" (orbit center: {x-self.orbit_radius:+.1f}, {y:+.1f})" if self.enable_orbits else ""
            print(f"  {i+1}. {self.vertex_names[i]}: ({x:+.1f}, {y:+.1f}){orbit_info}")
    ...
    def execute_mission_logic(self, target_z):
        ...
        for i, (vertex_x, vertex_y) in enumerate(self.vertices):
            ...
            try:
                ...
                # Check again before orbital photography
                if self.emergency_landing_triggered:
                    print(f" Emergency landing triggered - skipping orbital photography at vertex {i+1}")
                    break
                
                # Perform orbital photography
    ...
    def _perform_orbital_photography(self, center_x, center_y, center_z, vertex_num):
        ...
        try:
            # Calculate circle center so that the vertex is at 0 degrees (start) of the circle
            # If vertex is at 0 degrees, circle center is offset west by orbit_radius
            circle_center_x = center_x - self.orbit_radius
            circle_center_y = center_y
            
            print(f"   Vertex at ({center_x:.1f}, {center_y:.1f}) will be 0° of orbit")
            print(f"   Circle center at ({circle_center_x:.1f}, {circle_center_y:.1f})")
            
            # Drone is already at the vertex, which is the orbit start position (0 degrees)
            # No need to move to a separate starting position
            ...
            if self.orbit_mode == "waypoint":
                self._perform_waypoint_orbit(circle_center_x, circle_center_y, center_z, vertex_num)
            else:  # velocity mode
                self._perform_velocity_orbit(circle_center_x, circle_center_y, center_z, vertex_num)
    ...
    def _perform_waypoint_orbit(self, center_x, center_y, center_z, vertex_num):
        ...
        print(f"    Starting from vertex (0°), orbiting around center ({center_x:.1f}, {center_y:.1f})")
        ...
        # Generate orbital waypoints starting from current position (0 degrees)
        # We're already at the 0-degree position (the vertex), so start from waypoint 1
        for i in range(1, num_waypoints + 1):  # Start from 1 since we're already at 0
            ...
            # Calculate angle for this waypoint (starting from current position at 0°)
            angle = (i / num_waypoints) * 2 * math.pi
            
            # Calculate waypoint position relative to circle center
            ...
            if i % 4 == 0 or photos_taken < self.photos_per_orbit:  # Log every 4th waypoint or when taking photos
                ...
                print(f"     Waypoint {i}/{num_waypoints} ({degrees:.0f}°): ({waypoint_x:.1f}, {waypoint_y:.1f})")
            ...
            try:
                ...
                if (i % photo_waypoints == 0 and 
                    ...
                    if photo_result:
                        ...
                        self.total_photos += 1
                        degrees = (angle * 180 / math.pi) % 360
                        print(f"     Photo {photos_taken}/{self.photos_per_orbit} taken at {degrees:.0f}° (waypoint {i})")
                ...
                print(f"     Waypoint {i} movement failed: {str(waypoint_error)}")
                # If waypoint movement fails, try to continue with next waypoint
                continue
        
        # Complete the orbit by returning to the starting vertex position (0°)
        start_vertex_x = center_x + self.orbit_radius  # 0 degrees position
        start_vertex_y = center_y
        print(f"     Completing orbit - returning to vertex (0°): ({start_vertex_x:.1f}, {start_vertex_y:.1f})")
        
        try:
            self.client.moveToPositionAsync(start_vertex_x, start_vertex_y, center_z, smooth_orbit_speed).join()
        except Exception as e:
            print(f"     Failed to return to start position: {e}")
    ...
    def _perform_velocity_orbit(self, center_x, center_y, center_z, vertex_num):
        ...
        # Start from 0 degrees (current vertex position)
        initial_angle = 0.0
        ...
        while True:
            ...
            # Calculate smooth orbital progression starting from 0 degrees
            progress = min(elapsed_time / orbit_duration, 1.0)  # Clamp to 1.0
            angle = initial_angle + (progress * 2 * math.pi)
            
            # Calculate smooth orbital position relative to circle center
            ...
            # Calculate tangential velocity (primary motion) for circular motion around center
            ...
            # Smooth camera pointing - point towards circle center
            ...
            if ((current_time - last_photo_time) >= photo_interval and 
                ...
                if photo_result:
                    ...
                    degrees = (angle * 180 / math.pi) % 360
                    print(f"     Photo {photos_taken}/{self.photos_per_orbit} taken at {degrees:.0f}° orbit")
            
            # High-frequency control loop for ultra-smooth motion
            time.sleep(0.01)  # 100Hz update rate for maximum smoothness
            self.update_distance_tracking()
        
        # Complete orbit by returning to vertex position (0 degrees)
        final_x = center_x + self.orbit_radius  # 0 degrees position
        final_y = center_y
        print(f"   Completing orbit - returning to vertex (0°): ({final_x:.1f}, {final_y:.1f})")
        
        # Gentle stop - gradually reduce velocity while moving to final position
        print(f"   Completing orbit - gentle stop at vertex")
        for i in range(10):
        ...
        # Final stop and position correction
        self.client.moveByVelocityAsync(0, 0, 0, 0.1).join()
        
        # Ensure we end up back at the vertex position
        try:
            self.client.moveToPositionAsync(final_x, final_y, center_z, 2.0).join()
        except Exception as e:
            print(f"     Failed to return to vertex position: {e}")
    ...
    def get_planned_distance(self):
        ...
        # Orbital photography distance (circumference * number of orbits)
        orbit_circumference = 2 * math.pi * self.orbit_radius
    ...
    def get_mission_specific_metrics(self):
        ...
        if self.enable_orbits:
            metrics.update({
                ...
                "Orbit Pattern": "Vertex-centered (0° start)"
            })
        else:
            metrics.update({
                "Flight Pattern": "Simple box (no orbits)",
                "Orbital Photography": "Disabled"
            })
...
def main():
    ...
    parser.add_argument('--disable_orbits', action='store_true', default=False,
                       help='Disable orbital photography - simple box pattern only')
    
    args = parser.parse_args()
    
    # Handle enable/disable orbits logic
